<template>
  <v-row justify="center" align="center">
    <v-col cols="12" sm="8" md="6">
      ...
      <p class="text-body-1 text-justify">
        La expulsión de un proceso que está en el CPU ocurre cuando llega un proceso de menor tiempo de procesamiento. La expulsión consiste en enviar el proceso de nuevo a la cola de listos.
      </p>
      <p class="text-body-1 text-justify">
      ...
      <v-row class="mt-6" no-gutters>
        ...
        <v-col cols="12">
          <p>Cantidad de swaps/expulsiones realizados: {{ swaps }} </p>
        </v-col>
      </v-row>
      <v-row>
  ...
  methods: {
    ...
    stepHandler () {
      ...
      if (shouldCreateNewProcess) {
        this.n += 1
        this.ready.push({ id: this.n, burstTime: Math.floor(Math.random() * 10) + 1, waitTime: 0, cpuTime: 0, ioTime: 0, isPaying: false })
      }
      ...
      if (this.cpu.length === 0 && this.ready.length > 0) {
        ...
        // Expulsión swap
        if (this.ready.length > 0 && this.ready[0].burstTime < this.cpu[0].burstTime) {
          this.swaps += 1
          this.ready.push(this.cpu.shift())
          this.cpu.push(this.ready.shift())
        }
